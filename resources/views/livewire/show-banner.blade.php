                    ...
                    @foreach ($banners as $banner)
                        <div class="single-slide slick-slide" @if(isset($banner->params['bg-main'])) style="background-size: contain; background-image:url('storage/images/{{$banner->params['bg-main']??''}}')" @endif>
                            <div class="main-slider-content">
                                ...
                                <h1 class="title" @mobile style="font-size:{{$banner->params['main-text-mobile-size-px']??''}}px" @elsemobile style="font-size:{{$banner->params['main-text-size-px']??''}}px" @endmobile>{!!$banner->params['main-text'][App()->currentLocale()??config('shop.languages.default')]??''!!}</h1>
                                <div class="shop-btn">
